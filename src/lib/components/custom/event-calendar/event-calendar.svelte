<script lang="ts">
	...
	let currentWeeksMondayDate = $state(getCurrentWeeksMonday());

	//days and hours for the calendar grid
	const dayDates = $derived(Array.from({ length: 7}, (_, i ) => getDateOfDay(currentWeeksMondayDate, i)))
...
<EventCalendarNav 
    lastWeek={() => (currentWeeksMondayDate = moveWeek(currentWeeksMondayDate, -7))} 
    currentWeek={() => { currentWeeksMondayDate = getCurrentWeeksMonday(); }} 
    nextWeek={() => (currentWeeksMondayDate = moveWeek(currentWeeksMondayDate, 7))} 
    date={currentWeeksMondayDate} />    
<EventCalendarHeader days={dayDates}/>
<div class="calendar">
    <!-- the time column -->
